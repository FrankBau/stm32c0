...
// FLASH_SIZE_DEFAULT is a device specific constant
// FLASH_SIZE is read at runtime form the engineering bytes (is not a C const)
#define COUNTER_PAGE    ((FLASH_SIZE)/(PAGE_SIZE)-1u)      // last page of the flash memory

#define COUNTER_ADDR    (FLASH_BASE + COUNTER_PAGE * PAGE_SIZE)
...
int main(void)
{
    ...
    uint64_t counter = *(volatile uint64_t*)COUNTER_ADDR;
    counter++;

    // writing to flash memory is a bit more complicated
    flash_unlock();
    flash_erase_page(COUNTER_PAGE);
    flash_programn_doubleword(COUNTER_ADDR, counter);
    flash_lock();
